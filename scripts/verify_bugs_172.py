"""Regression: alert_reminder long backoff must not block due closed-summary."""
import os
import sys
import time
...
def _setup_rows(ds, a, *, now, future_retry):
    ...
    ds.enqueue_webhook_outbox(
        delivery_id="rem", target_id="t", incident_id="INC",
        incident_seq=1, event="alert_reminder", order_key="t",
        payload={
            "event": "alert_reminder", "target": "GW", "ip": "10.0.0.1",
            "status": "red", "message": "连接仍未恢复", "extra": {},
            "gate": ["reminder", "t", 1], "event_ts": red_ts + 1,
        },
        event_ts=red_ts + 1, max_attempts=12,
    )
    ds.enqueue_webhook_outbox(
    ...
    with ds._outbox_lock:
        ...
        conn.execute(
            ...
            "last_error='reminder timeout', delivery_state='pending' "
            "WHERE delivery_id='rem'",
            (future_retry,))
        conn.execute(
...
def test_reminder_backoff_unblock_fetch():
    ...
    ok = (
        ...
        and rows["rem"]["delivery_state"] == "dropped_stale"
    )
    print(
        f"Bug172 reminder backoff unblock fetch -> {ok} dropped={dropped} "
        f"due={[r['delivery_id'] for r in due]} "
        f"red={rows['red']['delivery_state']} rem={rows['rem']['delivery_state']}")
    return ok


def test_reminder_backoff_dispatch_closed_summary():
    from scripts.webhook_test_util import make_alerter
    a, disp, ds = make_alerter()
    now = time.time()
    future_retry = now + 300
    ...
    print(f"Bug172 reminder backoff dispatch -> {ok} sent={sent}")
    return ok


def main():
    results = [
        ("fetch", test_reminder_backoff_unblock_fetch()),
        ("dispatch", test_reminder_backoff_dispatch_closed_summary()),
    ]
    failed = [n for n, ok in results if not ok]
    if failed:
        print("FAILED:", failed)
        sys.exit(1)
    print("All bug 172 checks passed.")
