"""Regression checks for bug 124 (semantic reseed snapshot before update_target)."""
import os
import sys
import tempfile
...
def test_live_dict_semantic_diff_lost_after_update():
    ...
    print(f"Bug124 pre_persist semantic diff true, post false -> {ok}")
    return ok
...
def test_on_edit_uses_pre_persist_semantic_snapshot():
    ...
    print(f"Bug124 _on_edit snapshots semantic view before update_target -> {ok}")
    return ok
...
def main():
    ...
    if failed:
        print("FAILED:", failed)
        raise SystemExit(1)
    print("All bug 124 checks passed.")
