"""Regression: post-HTTP gate verify fail must still supersede red (Bug184)."""
import os
import sys
import time
...
def test_post_http_gate_fail_still_supersedes_red():
    ...
    print(
        f"Bug184 post_http_gate_fail_supersedes_red -> {ok} "
        f"side_effect_count={side_effect_count[0]} rec={rec} red={red}")
    return ok


def main():
    if not test_post_http_gate_fail_still_supersedes_red():
        print("FAILED")
        sys.exit(1)
    print("All bug 184 checks passed.")
